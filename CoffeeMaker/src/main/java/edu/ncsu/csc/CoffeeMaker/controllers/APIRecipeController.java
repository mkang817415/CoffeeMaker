...
public class APIRecipeController extends APIController {
    ...
    public ResponseEntity createRecipe ( @RequestBody final Recipe recipe ) {
    	...
    	if ( recipe.getIngredients().size() == 0 ) return new ResponseEntity( errorResponse(
    			"Recipe cannot have no ingredients."), HttpStatus.BAD_REQUEST);
     ...
     * the recipe to update (as a path variable)
     * 
     * @param name 
     *           The name of the Recipe to update
     * @param recipe 
     *          The new Recipe to update
     * @return Success if the recipe could be edited; an error if the recipe
     *        does not exist
     */

    @PutMapping ( BASE_PATH + "/recipes/{name}" )
    public ResponseEntity editRecipe ( @PathVariable String name, @RequestBody final Recipe recipe ) {
